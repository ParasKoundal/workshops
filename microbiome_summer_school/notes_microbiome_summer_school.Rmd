...
Here, we will begin to explore the Saanich Inlet data using `phyloseq`.
...
No this is not very informative as it has all 4000+ OTUs and is generally unreadable. The beauty of the `phyloseq` object, however, is that it contains everything - OTUs, taxonomy, metadata, and the tree - so we can easily add a color aesthetic from other data pieces onto the tree.

First, we'll color by domain to see how the tree did at separately out sequences at this high taxonomic level.
```{r}
...
**Try changing the taxonomic level and see how levels like Phylum or Class look!**

You can also learn more about the parameters available for `plot_tree` (or any function) using the help `?` function.
...
Plotting the transformed data and added a `geom` to pretty up the bars by removing all the blanks lines between individual OTUs, we can start to see how the communities change with depth.
```{r}
...
Finally, we'll delve into a more specific microbial question by sub-setting our data to 1 bacterial genus and looking to see if it correlates with any of our geochemical measurements.

First, we subset the data, still working in `phyloseq`.
```{r}
nitrospina = subset_taxa(saanich_percent, Genus=="Nitrospina")

nitrospina
```

We see that we've cut the data down to just the 8 OTUs classified as *Nitrospina*.
...
```{r}
plot_heatmap(nitrospina, method=NULL, sample.label="NO3_uM", sample.order="NO3_uM")
```

As expected *Nitrospina* OTUs tend to be more highly abundant when NO~3~ is abundant. 

**Try changing the genus and/or metadata variable to explore other questions!**
